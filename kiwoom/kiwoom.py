...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.screen_calculate_stock = "3000"
        ...
        # self.detail_account_info_event_loop = None  # 전역 변수로 만들어 줘서 멀티로 조회시 에러 방지
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == '예수금상세현황요청':
            ...
            ok_deposit = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, 0, "출금가능금액")
            self.logging.logger.debug("출금 가능 금액 : %s" % str(ok_deposit))
            self.detail_account_info_event_loop.exit()

        if sRQName == '계좌평가잔고내역요청':
            total_buy_money = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, 0, "총매입금액")
            total_buy_money_result = int(total_buy_money)
            print('총매입금액 %s' % total_buy_money_result)
            total_profit_loss_rate = self.dynamicCall("GetCommData(QString, QString, int, String)", sTrCode, sRQName, 0, "총수익률(%)")
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == '주식체결':
            ...
            self.logging.logger.debug("[체결] 종목코드:%s, 체결시간:%s, 현재가:%s 전일대비:%s 등락율:%s" %
                                      (sCode, a, str(b), str(c), str(d)))
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                #                                   sCode, qty, e, self.realType.SENDTYPE['거래구분']['지정가'], ""])
                order_success = self.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                                                 ["신규매수", self.portfolio_stock_dict[sCode]['주문용스크린번호'], self.account_num, 1,
                                                  sCode, qty, 0, self.realType.SENDTYPE['거래구분']['시장가'], ""])
                if order_success == 0:
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFIdList):
        if int(sGubun) == 0:
            ...
            not_chegual_qty = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['미체결수량'])
            not_chegual_qty = int(not_chegual_qty)
            order_gubun = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문구분'])
            order_gubun = order_gubun.strip().lstrip('+').lstrip('-')
            che_time_str = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문/체결시간'])
            che_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['체결가'])
            if che_price == '':
                che_price = 0
            else:
                che_price = int(che_price)

            che_qty = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['체결량'])
            if che_qty == '':
                che_qty = 0
            else:
                che_qty = int(che_qty)
            cur_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['현재가'])
            ...
            self.not_account_stock_dict[order_number].update({'주문수량': order_qty})
            self.not_account_stock_dict[order_number].update({'주문가격': order_price})
            self.not_account_stock_dict[order_number].update({'미체결수량': not_chegual_qty})
            ...
            self.not_account_stock_dict[order_number].update({'주문/체결시간': che_time_str})
            self.not_account_stock_dict[order_number].update({'체결가': che_price})
            self.not_account_stock_dict[order_number].update({'체결량': che_qty})
            self.not_account_stock_dict[order_number].update({'현재가': cur_price})
        ...
        if int(sGubun) == 1:
            ...
            stock_name = stock_name.strip()
            cur_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['현재가'])
            cur_price = abs(int(cur_price))
            ...
            self.jango_dict[sCode].update({'현재가': cur_price})
            self.jango_dict[sCode].update({'종목코드': sCode})
            self.jango_dict[sCode].update({'종목명': stock_name})
            self.jango_dict[sCode].update({'보유수량': stock_qty})
            self.jango_dict[sCode].update({'주문가능수량': like_qty})
